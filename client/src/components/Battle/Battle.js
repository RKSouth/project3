...
class Battle extends Component {
  ...
  attack = (event, attackIndex) => {
    this.setState(
      {
        userAttack: attackIndex,
        cpuAttack: Math.floor(Math.random() * attack.length),
      },
      this.attackRound
    );
  };
  ...
  attackButtons = () => {
    return (
      <div>
        <button className="button" onClick={() => this.attack('x', 0)}>
          {this.state.characterAttacks[0].name}
        </button>
        <button className="button" onClick={() => this.attack('y', 1)}>
          {this.state.characterAttacks[1].name}
        </button>
        <button className="button" onClick={() => this.attack('z', 2)}>
          {this.state.characterAttacks[2].name}
        </button>
        <button className="button" onClick={() => this.attack('a', 3)}>
          {this.state.characterAttacks[3].name}
        </button>
        <button className="button" onClick={() => this.attack('b', 4)}>
          {this.state.characterAttacks[4].name}
        </button>
      </div>
    );
  };
      ...
      this.setState(
        {
          ...
          message: 'YOU DEALT A MIGHTY BLOW! MINUS ONE TO YOUR OPPONENT!',
        },
        checkWinner
      );
    } else if (
      ...
      this.setState(
        {
          ...
          message: 'A LEGENDARY STRIKE! MINUS TWO TO YOUR OPPONENT!',
        },
        checkWinner
      );
    } else if (
      ...
      this.setState(
        {
          ...
          message: 'YOUR ATTACK HAS BEEN FOILED! MINUS ONE TO YOU!',
        },
        checkWinner
      );
    } else if (
      ...
      this.setState(
        {
          ...
          message: 'A STUNNING WALLOP BY YOUR OPPONENT! MINUS TWO TO YOU!',
        },
        checkWinner
      );
      ...
      <div>
        <div className='message'>
          ...
          {(winner === 'cpu' || winner === 'Player') && (
            <div className="winLoseImg">
              {winner === 'cpu' && (
                <img src="./Images-char/youLose.png" id="img" alt="loser" />
              )}
              {winner === 'Player' && (
                <img src="./Images-char/youWin.png" id="img" alt="winner" />
              )}
            </div>
          )}
        </div>
        <div className='container'>
          <div className="user-corner">
            ...
            <div>
              <h5>
                {playerCharacter.name} Health: {userHealth}
              </h5>
            </div>
          </div>
          <div className="cpu-corner">
            <div>
              <img
                className="cpu-img"
                src={cpuCharacter.image}
                alt="computer character"
              ></img>
            </div>
            <div>
              <h5>
                {cpuCharacter.name} Health: {cpuHealth}
              </h5>
